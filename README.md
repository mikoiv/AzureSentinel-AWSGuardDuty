...
There are alternative ways to do this integration, namely with Amazon CloudWatch. The S3 export + Logstash method is easy to implement, but at least one downside is the time delay to process updated findings (15min at minimum). Heavy AWS users with streamlined SecOps processes will want to evaluate the options carefully.
